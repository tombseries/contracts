...
// File: contracts/MetadataUpdate.sol
...
contract MetadataUpdate {
    ...
    returns (string memory _infoUrl)
  {
    string memory id = uint2str(_tokenId);
    return
      base.toSlice().concat(id.toSlice()).toSlice().concat(".json".toSlice());
  }
...
contract Shadow is Folia, ERC2981ContractWideRoyalties {
  ...
  constructor(
    address _tombCouncil,
    address _metadata,
    address royaltyRecipient
  ) public Folia("House SHADOW", "SHD", _metadata) {
  ...
  function transferFrom(
    ...
    require(_isApprovedOrOwner(msg.sender, tokenId));
    require(
      !IShadowController(controller).isLocked(from, to, tokenId),
      "SHADOW is locked"
    );
    _transferFrom(from, to, tokenId);
  }
